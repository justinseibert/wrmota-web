...
function initMap() {
  ...
  config.search_box = new google.maps.places.SearchBox(input);
  map.controls[google.maps.ControlPosition.TOP_LEFT].push(input);

  // Bias the SearchBox results towards current map's viewport.
  map.addListener('bounds_changed', function() {
    config.search_box.setBounds(map.getBounds());
  });

  // Listen for the event fired when the user selects a prediction and retrieve
  // more details for that place.
  config.search_box.addListener('places_changed', searchMap);
...
function searchMap(){
  var places = config.search_box.getPlaces();
  ...
  config.search_markers.forEach(function(marker) {
    marker.setMap(null);
  });
  config.search_markers = [];
